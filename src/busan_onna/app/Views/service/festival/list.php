...
<head>
    ...
    if (!empty($activeDistrict)) {
        $seoTitle = esc($activeDistrict) . ' 축제·행사 | 부산온나';
        $seoDesc  = '부산 ' . esc($activeDistrict) . ' 지역 축제·행사 목록. 해당 지역에서 열리는 문화 행사를 한눈에 확인하세요.';
    } elseif (!empty($activeCategory) && isset($categories[(int)$activeCategory])) {
        $catName  = $categories[(int)$activeCategory];
        $seoTitle = esc($catName) . ' 축제 | 부산온나';
        $seoDesc  = '부산 ' . esc($catName) . ' 분야 축제·행사 목록. 부산의 다양한 ' . esc($catName) . ' 행사 일정을 확인하세요.';
    } elseif ($activeIsFree === '1') {
        $seoTitle = '부산 무료 축제·행사 | 부산온나';
        $seoDesc  = '부산에서 열리는 무료 축제·행사 목록. 입장료 없이 즐길 수 있는 부산의 문화 행사를 확인하세요.';
    } else {
        $seoTitle = '부산 축제·행사 | 부산온나 - 부산 여행의 시작';
        $seoDesc  = '부산의 다채로운 문화 축제와 행사 정보를 한눈에 확인하세요. 음악, 미술, 해양, 음식 등 다양한 부산 축제 일정을 알려드립니다.';
    }
    ...
    if ($activeIsFree !== '')       $canonParams[] = 'is_free=' . rawurlencode($activeIsFree);
    if ($currPage > 1)              $canonParams[] = 'page=' . $currPage;
    $canonicalUrl = 'https://busanonna.com/festivals' . (!empty($canonParams) ? '?' . implode('&', $canonParams) : '');
    ...
    <meta name="keywords"    content="부산축제, 부산행사, 부산문화행사, 부산이벤트, 부산여행, 부산무료축제, 부산온나">
    ...
    <meta property="og:image"        content="https://busanonna.com/img/og-festival.jpg">
    ...
    <meta name="twitter:image"       content="https://busanonna.com/img/og-festival.jpg">
    ...
    foreach (($festivals ?? []) as $i => $f) {
        $item = [
            ...
            'name'     => $f['name'],
            'url'      => 'https://busanonna.com/festivals/' . (int)$f['idx'],
        ];
        // 축제는 시작·종료일 포함
        if (!empty($f['start_date'])) $item['startDate'] = $f['start_date'];
        if (!empty($f['end_date']))   $item['endDate']   = $f['end_date'];
        $ldItems[] = $item;
    }
    echo json_encode([
        ...
        <div class="restaurant-grid" id="cardView">
            ...
            <a class="r-card" href="/festivals/<?= (int)$f['idx'] ?>" style="text-decoration:none;color:inherit;">
                ...
                <div class="r-card-body">
                    ...
                    */ ?>
                    <!-- 좋아요 카운트 (0이면 표시 안 함) -->
                    <?php if (($f['like_cnt'] ?? 0) > 0): ?>
                    <div class="r-card-likes">♥ <?= (int)$f['like_cnt'] ?></div>
                    <?php endif; ?>
            ...
            <a class="r-list-item" href="/festivals/<?= (int)$f['idx'] ?>" style="text-decoration:none; color:inherit;">
                ...
                <div class="r-list-body">
                    <div class="r-list-header">
                        ...
                        */ ?>
                        <?php if (($f['like_cnt'] ?? 0) > 0): ?>
                        <span class="r-list-likes">♥ <?= (int)$f['like_cnt'] ?></span>
                        <?php endif; ?>
                    </div>
